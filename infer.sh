#!/usr/bin/env bash
# Example inference script for running ACT policy on the AgileX robot and recording eval episodes.
...
# Example:
#   TRAIN_RUN=act_agilex_215_multi CHECKPOINT_STEP=100000 ./infer.sh
#   TRAIN_RUN=act_agilex_215_multi CHECKPOINT_STEP=60000  ./infer.sh  # auto-pads to 060000
TRAIN_RUN="${TRAIN_RUN:-act_agilex_215_multi}"
CHECKPOINT_STEP="${CHECKPOINT_STEP:-last}"
if [[ "${CHECKPOINT_STEP}" =~ ^[0-9]+$ ]]; then
  CHECKPOINT_STEP="$(printf "%06d" "$((10#${CHECKPOINT_STEP}))")"
fi

log_file="${LOG_DIR}/infer_act_${TRAIN_RUN}_${CHECKPOINT_STEP}_${timestamp}.log"

# Paths
CHECKPOINT_DIR="outputs/train/act_agilex_215/checkpoints/100000/pretrained_model"
...
lerobot-record \
  ...
  --dataset.single_task="ACT agilex eval ${TRAIN_RUN} checkpoint ${CHECKPOINT_STEP}" \
