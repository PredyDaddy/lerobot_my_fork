...
log_file="${LOG_DIR}/train_act_multi_${timestamp}.log"

IFS="," read -ra CUDA_DEVICES <<< "${CUDA_VISIBLE_DEVICES}"
NUM_PROCESSES="${#CUDA_DEVICES[@]}"
...
accelerate launch \
  ...
  --policy.type=act \
  --output_dir="${ROOT_DIR}/outputs/train/act_agilex_215_multi" \
  --job_name=act_agilex_215_multi \
  --policy.device=cuda \
