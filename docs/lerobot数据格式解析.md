...
> 本文档详细解析 LeRobot Dataset v3.0 的完整技术规范，包括目录结构、元数据格式、数据存储、视频编解码、统计计算、API 使用等核心内容。
...
3. [目录结构详解](#3-目录结构详解)
4. [元数据文件规范](#4-元数据文件规范)
5. [数据类型与特征系统](#5-数据类型与特征系统)
6. [时序数据处理](#6-时序数据处理)
7. [视频与图像处理](#7-视频与图像处理)
8. [数据存储优化](#8-数据存储优化)
9. [统计计算](#9-统计计算)
10. [核心类与 API](#10-核心类与-api)
11. [高级功能](#11-高级功能)
12. [流式数据集](#12-流式数据集)
13. [最佳实践](#13-最佳实践)
14. [版本兼容性](#14-版本兼容性)
...
| `float64` | 64位浮点数 | Parquet 列 | 高精度数值 |
| `int64` | 64位整数 | Parquet 列 | 索引、计数 |
| `int32` | 32位整数 | Parquet 列 | 较小范围的整数 |
| `bool` | 布尔值 | Parquet 列 | 完成标记 |
| `string` | 字符串 | Parquet 列 | 任务描述、元信息 |
| `image` | 单张图像 | PNG文件 | 静态图像 |
